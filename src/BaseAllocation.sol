 ...
 *
 * @notice     Basic Token Allocation which is inherited by the other MetaVesT types
 **/
abstract contract BaseAllocation is ReentrancyGuard, SafeTransferLib {
    // enum to determine which tokens in the contract will be counted towards governing power
    enum GovType {
        all,
        vested,
        unlocked
    ...
    event MetaVesT_TransferabilityUpdated(
        address indexed grantee,
        bool isTransferable
    );
    event MetaVest_TransferRightsPending(
        address indexed grantee,
        address indexed pendingGrantee
    );
    event MetaVesT_TransferredRights(
        address indexed grantee,
        address transferee
    );
    event MetaVesT_UnlockRateUpdated(
        address indexed grantee,
        uint208 unlockRate
    );
    event MetaVesT_VestingRateUpdated(
        address indexed grantee,
        uint208 vestingRate
    );
    event MetaVesT_Withdrawn(
        address indexed grantee,
    ...
    function getMajorityVotingPower()
        external
        view
        returns (uint256 majorityVotingPower)
    {
    ...
    function confirmMilestone(uint256 _milestoneIndex) external nonReentrant {
        if (terminated) revert MetaVesT_AlreadyTerminated();
        if (_milestoneIndex >= milestones.length)
            revert MetaVesT_MilestoneIndexOutOfRange();
        ...
        for (uint256 i; i < milestone.conditionContracts.length; ++i) {
            if (
                !IConditionM(milestone.conditionContracts[i]).checkCondition(
                    address(this),
                    msg.sig,
                    _data
                )
    ...
    function addMilestone(
        ...
        if (milestones.length >= MAX_MILESTONES)
            revert MetaVesT_MaxMilestonesReached();
    ...
    function withdraw(uint256 _amount) external nonReentrant onlyGrantee {
        ...
        if (
            _amount > getAmountWithdrawable() ||
            _amount > IERC20M(allocation.tokenContract).balanceOf(address(this))
