...
contract BaseScript is Script {
    ...
    MockERC20 token;
    MockERC20Stable paymentToken;


     function run() public {
            deployerAddress = vm.addr(vm.envUint("PRIVATE_KEY_DEPLOY"));
            uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY_DEPLOY");
            address testGrantee = 0x9e97c5a6fb1770bb13fc8cFEb716903fbdF1EF33;
            
           
            vm.startBroadcast(deployerPrivateKey);
             token = new MockERC20("Test Token", "TT");
              token.mint(deployerAddress, 1000000e70);
            ...
            for(int i=0; i<70; i++){
                vm.startBroadcast(deployerPrivateKey);
                //(metavestType _type, address _grantee,  BaseAllocation.Allocation calldata _allocation, BaseAllocation.Milestone[] calldata _milestones, uint256 _exercisePrice, address _paymentToken,  uint256 _shortStopDuration, uint256 _longStopDate) external onlyAuthority conditionCheck returns (address)
                 BaseAllocation.Allocation memory allocation = BaseAllocation.Allocation({
                    tokenContract: address(token),
                    tokenStreamTotal: 10e18,
                    vestingCliffCredit: 1e18,
                    unlockingCliffCredit: 1e18,
                    vestingRate: 1e18,
                    ...
                    BaseAllocation.Milestone[] memory milestones = new BaseAllocation.Milestone[](14);
                    ...
                    token.approve(address(metaVesTController), 100100e18);

                    metavestController(metaVesTController).createMetavest(
